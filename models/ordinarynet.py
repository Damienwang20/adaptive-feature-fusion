...
def conv_bn_activate(in_channel, out_channel, kernel, padding, activate='p_relu'):
    if activate is not None:
        conv_bn = nn.Sequential(
            nn.Conv1d(in_channel, out_channel, kernel, padding=padding),
...
class ConvResBlock(nn.Module):
    def __init__(self, in_channel, out_channel, kernel, padding=0, activate='p_relu'):
        super().__init__()
        
        self.conv2 = conv_bn_activate(in_channel, out_channel, kernel, padding, activate)
        self.conv3 = conv_bn_activate(out_channel, out_channel, kernel, padding, None)
...
class OrdinaryResNet(BaseModel):
    def __init__(self, seq_len, n_class, in_channel=1, out_channel=64, 
        ...
        self.block1 = nn.Sequential(
            ConvResBlock(self.in_channel, self.out_channel, kernel_list[0], padding[0], activate),
            nn.MaxPool1d(2),
            SELayer(self.out_channel),
            nn.Dropout(0.3))
        self.block2 = nn.Sequential(
            ConvResBlock(self.out_channel, self.out_channel*2, kernel_list[1], padding[1], activate),
            nn.MaxPool1d(2),
            SELayer(self.out_channel*2),
            nn.Dropout(0.3))
        self.block3 = nn.Sequential(
            ConvResBlock(self.out_channel*2, self.out_channel*2, kernel_list[2], padding[2], activate),
            nn.MaxPool1d(2),
            SELayer(self.out_channel*2),
            nn.Dropout(0.3))
...
UCR：
"""
class OrdinaryFusionNet(BaseModel):
    def __init__(self, seq_len, n_class, 
        ...
        self.cnn_feature_extractor = OrdinaryResNet(seq_len, n_class, in_channel, out_channel,
                                                kernel_list, padding, activate)
...
def dump_embedding(proto_embed, sample_embed, labels=None, logits=None, dump_file='./embeddings.txt'):
    import numpy as np
